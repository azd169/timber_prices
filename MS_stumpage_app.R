    ...
    column(12,
          tags$div(style = "display: flex; align-items: flex-start; gap: 10px; margin-top: 20px; font-size: 20px;",
                    tags$div(style = "flex: 1;",
                             ...
                             uiOutput("quarter_selector"), # Dynamic UI for selecting quarters
...
server <- function(input, output, session) {
  ...
  output$plot_ui <- renderUI({
    ...
    plot_height <- input$window_height - 300  # Adjust this value as needed
  ...
  output$plot <- renderPlotly({
    ...
    p1 <- ggplot(data,
                 aes(x = Time,
                     ...
                     color = Type,
                     shape = Type,
                     text = paste("Type:", Type, "<br>Time:", Time, "<br>Price ($/ton):", Average)))  +
